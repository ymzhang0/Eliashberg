...
function compute_dispersion_surface_data(disp::Dispersion, kgrid::KGrid{2})
    return with_stage_log(
        ...
        context=(model=model_summary(disp), grid=grid_summary(kgrid)),
...
function compute_band_data(disp::ElectronicDispersion, kpath::KPath{D}) where {D}
    return with_stage_log(
        "Compute band data";
        context=(model=model_summary(disp), kpath=kpath_summary(kpath)),
...
function compute_fermi_surface_volume(disp::ElectronicDispersion; n_pts::Integer=100)
    return with_stage_log(
        ...
        context=(model=model_summary(disp), n_pts=Int(n_pts)),
...
function compute_phase_transition_data(
    ...
    return with_stage_log(
        ...
        context=(field=field_summary(field), model=model_summary(model), interaction=interaction_summary(interaction), kgrid=grid_summary(kgrid), approx=approx_summary(approx), n_phis=length(phis), n_temperatures=length(Ts), warm_start=warm_start),
...
function compute_renormalized_band_data(
    ...
    return with_stage_log(
        ...
        context=(field=field_summary(field), model=model_summary(model), interaction=interaction_summary(interaction), kgrid=grid_summary(kgrid), kpath=kpath_summary(kpath), approx=approx_summary(approx), n_temperatures=length(Ts), warm_start=warm_start),
...
function compute_zeeman_pairing_data(
    ...
    return with_stage_log(
        ...
        context=(model=model_summary(model), interaction=interaction_summary(interaction), kgrid=grid_summary(kgrid), T=Float64(T_val), h=Float64(h_val), n_q=length(q_vals), approx=approx_summary(approx), warm_start=warm_start),
