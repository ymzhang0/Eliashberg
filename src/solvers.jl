...
function renormalized_dispersion(disp::ElectronicDispersion, Σ::SelfEnergy, k::SVector{D, Float64}, ω::Float64) where {D}
    ε0 = ε(k, disp)           
    Σval = Σ(k, ω)         
...
function evaluate_V(k::SVector{D, Float64}, kp::SVector{D, Float64}, interaction::ConstantInteraction{D}, disp::ElectronicDispersion) where {D}
    return V(interaction)
end

function evaluate_V(k::SVector{D, Float64}, kp::SVector{D, Float64}, interaction::YukawaInteraction{D}, disp::ElectronicDispersion) where {D}
    return V(k, kp, interaction)
end

function evaluate_V(k::SVector{D, Float64}, kp::SVector{D, Float64}, interaction::Interaction, disp::ElectronicDispersion) where {D}
